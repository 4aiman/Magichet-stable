...
public class MagichetAssetCopy extends Activity
{
	@Override
	public void onCreate(Bundle savedInstanceState)
	{
		...
		MagichetAssetCopy prevActivity = 
				(MagichetAssetCopy) getLastNonConfigurationInstance();
	...
	private class copyAssetTask extends AsyncTask<String, Integer, String>
	{
		...
		protected String doInBackground(String... files)
		{
			...
			// prepare temp folder
			File TempFolder = new File(baseDir + "magichet/tmp/");
			...
			if (!TempFolder.exists())
			{
				...
				{
					Log.v("MagichetAssetCopy","deleting: " + todel[i].getAbsolutePath());
					todel[i].delete();
				}
			}
			...
			// add a .nomedia file
			try {
				OutputStream dst = new FileOutputStream(baseDir + "Magichet/.nomedia");
				dst.close();
			} catch (IOException e) {
				Log.e("MagichetAssetCopy","Failed to create .nomedia file");
				e.printStackTrace();
			}
			...
			for (int i = 0; i < m_tocopy.size(); i++)
			{
				try
				{
					...
					try
					{
						...
						Log.e("MagichetAssetCopy","Copying file: " + filename + " FAILED (not in assets)");
						e.printStackTrace();
						continue;
					}
					...
					if (len > 0)
					{
						...
						try
						{
							...
							Log.e("MagichetAssetCopy","Copying file: " + baseDir +
						...
						dst.close();
						Log.v("MagichetAssetCopy","Copied file: " +
									m_tocopy.get(i) + " (" + total_filesize +
									" bytes)");
					}
					else if (len < 0)
					{
						Log.e("MagichetAssetCopy","Copying file: " +
			...
			while (itr.hasNext())
			{
					...
					if (!current_folder.exists())
					 {
						...
						{
							Log.e("MagichetAssetCopy","\t failed create folder: " +
									FlashPath);
						}
						else
						{
							Log.v("MagichetAssetCopy","\t created folder: " +
									FlashPath);
						}
					}
				...
				if (testme.exists())
				{
					...
					catch (IOException e)
					{
						...
						Log.e("MagichetAssetCopy","Failed to open asset file \"" +
		...
		protected void BuildFolderList()
		{
			...
			{
				Log.e("MagichetAssetCopy","Error on processing index.txt");
				e1.printStackTrace();
			}
		}
		...
		protected void BuildFileList()
		{
			...
			{
				Log.e("MagichetAssetCopy","Error on processing filelist.txt");
				e1.printStackTrace();
			}
		}
